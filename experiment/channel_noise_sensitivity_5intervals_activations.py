"""
Channel Noise Sensitivity Analysis - 5-Interval Division (Activation-based)
============================================================================
This version divides channels into 5 equal intervals based on activation importance:

  IMPORTANT: Channels are sorted by activation in DESCENDING order

  - [0-20%]: Top 20% (HIGHEST activation) - Most important
  - [20%-40%]: Upper-Mid 20%
  - [40%-60%]: Middle 20%
  - [60%-80%]: Lower-Mid 20%
  - [80%-100%]: Bottom 20% (LOWEST activation) - Least important
...
# ==========================================================
# Channel Division into 5 Intervals
# ==========================================================
def divide_channels_into_intervals(importance_scores):
    ...
    - Interval 0: indices [0:20%]     = Top 20% (HIGHEST activation)
    - Interval 1: indices [20%:40%]   = Upper-Mid 20%
    - Interval 2: indices [40%:60%]   = Middle 20%
    - Interval 3: indices [60%:80%]   = Lower-Mid 20%
    - Interval 4: indices [80%:100%]  = Bottom 20% (LOWEST activation)

    Returns:
        interval_indices: List of 5 tensors, each containing channel indices
    """
    num_channels = len(importance_scores)
    sorted_indices = torch.argsort(importance_scores, descending=True)

    # Each interval is 20% of channels
    interval_size = num_channels // 5

    intervals = []
    for i in range(5):
        start_idx = i * interval_size
        if i == 4:  # Last interval takes remaining channels
...
# Visualization
# ==========================================================
def plot_noise_sensitivity_curves_5intervals(results_dict, save_path):
    """Plot 6 curves: 5 intervals + baseline"""
    from scipy.interpolate import make_interp_spline

    fig, ax = plt.subplots(figsize=(14, 8))

    # Define colors and styles for 5 intervals + baseline
    # NOTE: Interval-1 = Top 20% (highest activation), Interval-5 = Bottom 20% (lowest activation)
    strategies_config = {
    ...
    ax.set_title('Impact of Channel Noise on Explainability: 5-Interval Analysis (Activation-based)',
...
def main():
    print("="*80)
    print("Channel Noise Sensitivity Analysis - 5-Interval Division")
    print("="*80)
    ...
    print(f"\nExperiment Configuration:")
    print(f"  Division: 5 intervals (20% each)")
    ...
    # Define 5 interval strategies + baseline
    interval_names = [
        'Interval-1 [0-20%]',
        'Interval-2 [20-40%]',
        'Interval-3 [40-60%]',
    ...
    # Run experiments
    print("\n" + "="*80)
    print("Step 4-6: Running 5-Interval Noise Sensitivity Experiments")
    print("="*80)
    ...
    for img_idx in range(NUM_TEST_IMAGES):
        ...
        for i, interval in enumerate(intervals):
            print(f"    Interval {i+1} [{i*20}-{(i+1)*20}%]: {len(interval)} channels")
    ...
    save_path = "./results/channel_noise_sensitivity_5intervals_activation.png"

    plot_noise_sensitivity_curves_5intervals(results_dict, save_path)
    ...
    print("\n  Interval Mapping:")
    print("  ─────────────────────────────────────────────────────────")
    print("  🔴 [0-20%]    = Top 20%      (HIGHEST activation)")
    print("  🟠 [20-40%]   = Upper-Mid 20%")
    print("  🔵 [40-60%]   = Middle 20%")
    print("  🟢 [60-80%]   = Lower-Mid 20%")
    print("  🟣 [80-100%]  = Bottom 20%   (LOWEST activation)")
    print("  ⚫ Baseline   = No Noise (reference)")
    print("  ─────────────────────────────────────────────────────────")
    print("\n  Expected behavior:")
    print("    - Top 20% noise → ROAD increases (explainability degrades)")
    print("    - Bottom 20% noise → ROAD stable (minimal impact)")
    print("\n" + "="*80)
    ...
    for interval_name in interval_names:
        ...
        print(f"  {interval_name:<25}: ROAD={road:.4f}±{road_std:.4f}, Acc={accuracy:.4f}±{accuracy_std:.4f}")

    baseline_road = results_dict['Baseline'][test_noise]['road']
    baseline_accuracy = results_dict['Baseline'][test_noise]['accuracy']
    print(f"  {'Baseline':<25}: ROAD={baseline_road:.4f}, Acc={baseline_accuracy:.4f}")
    ...
    print(f"  → This should be '[0-20%] Top (Highest Activation)'")
    print(f"\nLeast sensitive interval: {least_sensitive} (ROAD={interval_roads[least_sensitive]:.4f})")
    print(f"  → This should be '[80-100%] Bottom (Lowest Activation)'")
    ...
    if "Interval-1" in most_sensitive:
        print("✅ CORRECT: Top 20% (Interval-1) is most sensitive")
    else:
        print("⚠️  WARNING: Expected Interval-1 to be most sensitive!")

    if "Interval-5" in least_sensitive or "Interval-4" in least_sensitive or "Interval-3" in least_sensitive:
        print("✅ CORRECT: Lower intervals are least sensitive")
    else:
        print("⚠️  WARNING: Expected lower intervals to be least sensitive!")
    ...
    print(f"  - Interval-1 [0-20%] = Top (Highest Activation)")
    print(f"  - Interval-5 [80-100%] = Bottom (Lowest Activation)")
    print("\n")
