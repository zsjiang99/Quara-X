...
================================================================
This experiment tests which channel groups are most sensitive to noise IN THE EXPLANATION PROCESS ONLY.

KEY DESIGN:
  - Model: Always FP32, never modified
  - Forward/Backward: Always clean (no noise)
  - Noise injection: ONLY during GradCAM computation, ONLY on selected interval

EXPERIMENT:
  - Divide channels into 5 intervals by activation magnitude
  - Add noise to ONE interval at a time during GradCAM computation
  - Keep other intervals clean
  - Measure ROAD score degradation

QUESTION:
  "Which channels are most critical for explanation quality?"
  - If Interval-1 (high activation) gets noise → large ROAD drop?
  - If Interval-5 (low activation) gets noise → small ROAD drop?

Author: Selective Channel Noise Analysis
Date: 2025-10-30
Version: 1.0 (Correct Implementation)
"""
...
# ==========================================================
# Selective Noisy GradCAM
# ==========================================================
class SelectiveNoisyGradCAM:
    """
    GradCAM with selective channel noise

    CRITICAL:
    - Model always FP32
    - Forward/Backward always clean
    - Noise ONLY applied during CAM computation, ONLY to selected channels
    """
    ...
    def _add_selective_noise(self, tensor, channel_mask, noise_strength, normalize=True):
        """
        Add noise ONLY to selected channels

        Args:
            tensor: [B, C, H, W]
            channel_mask: [C] boolean, True = add noise to this channel
            noise_strength: scalar
            normalize: if True, noise is normalized by channel std

        Returns:
            noisy_tensor with noise only on selected channels
        """
        if noise_strength == 0 or channel_mask is None:
            return tensor
        ...
        for c in range(C):
            if channel_mask[c]:  # Only add noise to this channel
                ch_data = tensor[:, c, :, :]  # [B, H, W]

                if normalize:
                    ch_std = ch_data.std()
                    if ch_std > 0:
                        noise = torch.randn_like(ch_data) * ch_std * noise_strength
                    else:
                        noise = torch.zeros_like(ch_data)
                else:
                    noise = torch.randn_like(ch_data) * noise_strength

                noisy_tensor[:, c, :, :] = ch_data + noise

        return noisy_tensor

    def __call__(self, input_tensor, target_class, channel_mask=None, noise_strength=0.0, normalize=True):
        """
        Compute GradCAM with selective channel noise

        Args:
            input_tensor: input image
            target_class: target class for GradCAM
            channel_mask: [C] boolean mask, True = add noise to this channel
            noise_strength: noise level (σ)
            normalize: whether to normalize noise by channel std

        Flow:
            1. Clean forward → clean activation
            2. Clean backward → clean gradient
            3. Add noise ONLY to selected channels
            4. Compute CAM with noisy components
        """
        self.model.eval()
        self.model.zero_grad()
        ...
        # Step 3: Add noise ONLY to selected channels
        noisy_activations = self._add_selective_noise(
            clean_activations, channel_mask, noise_strength, normalize
        )
        noisy_gradients = self._add_selective_noise(
            clean_gradients, channel_mask, noise_strength, normalize
        )

        # Step 4: Compute GradCAM with noisy components
        weights = noisy_gradients.mean(dim=(2, 3), keepdim=True)  # [B, C, 1, 1]
        cam = (weights * noisy_activations).sum(dim=1, keepdim=True)  # [B, 1, H, W]
        cam = F.relu(cam)
...
def divide_channels_into_intervals(importance_scores):
    ...
    DESCENDING order:
    - Interval 0: [0-20%]     = Top 20% (HIGHEST activation)
    - Interval 1: [20-40%]    = Upper-Mid 20%
    - Interval 2: [40-60%]    = Middle 20%
    - Interval 3: [60-80%]    = Lower-Mid 20%
    - Interval 4: [80-100%]   = Bottom 20% (LOWEST activation)
    """
...
def create_channel_mask(num_channels, interval_indices):
    ...
    Args:
        ...
        interval_indices: indices of channels in this interval

    Returns:
        [C] boolean tensor, True for channels in interval
    """
...
# Visualization
# ==========================================================
def plot_selective_noise_sensitivity(results_dict, save_path):
    """
    Plot noise sensitivity for each interval

    X-axis: Noise strength (σ)
    Y-axis: ROAD Score
    ...
    interval_config = {
        'Interval-1 [0-20%]': {'color': '#E74C3C', 'linewidth': 3.0, 'marker': 'o', 'markersize': 8,
                               'label': '[0-20%] Top (Highest Activation)'},
        'Interval-2 [20-40%]': {'color': '#F39C12', 'linewidth': 2.8, 'marker': 's', 'markersize': 7,
                                'label': '[20-40%] Upper-Mid'},
        'Interval-3 [40-60%]': {'color': '#3498DB', 'linewidth': 2.8, 'marker': '^', 'markersize': 7,
                                'label': '[40-60%] Middle'},
        'Interval-4 [60-80%]': {'color': '#2ECC71', 'linewidth': 2.8, 'marker': 'v', 'markersize': 7,
                                'label': '[60-80%] Lower-Mid'},
        'Interval-5 [80-100%]': {'color': '#8E44AD', 'linewidth': 3.0, 'marker': 'D', 'markersize': 7,
                                 'label': '[80-100%] Bottom (Lowest Activation)'},
        'Baseline': {'color': '#34495E', 'linewidth': 2.5, 'marker': 'x', 'markersize': 8,
    ...
    for interval_name, style in interval_config.items():
        ...
        noise_levels = np.array(sorted(interval_results.keys()))
        road_scores = np.array([interval_results[noise]['road'] for noise in noise_levels])
    ...
    # Styling
    ax.set_xlabel('Noise Strength (σ) - Applied to Selected Interval Only', fontsize=16, fontweight='bold')
    ax.set_ylabel('ROAD Score (Explanation Quality)', fontsize=16, fontweight='bold')

    title = 'Selective Channel Noise Sensitivity: Which Channels are Critical for Explanation?'
    ...
    # Add annotation
    ax.text(0.98, 0.02,
            'Note: Model always FP32, noise only in GradCAM computation',
...
def main():
    print("="*80)
    print("Selective Channel Noise Sensitivity - GradCAM Explanation Only")
    print("="*80)
    ...
    all_images_results = {name: {noise: [] for noise in noise_levels}
    ...
    for img_idx in range(NUM_TEST_IMAGES):
        ...
        gradcam = SelectiveNoisyGradCAM(model, target_layer)

        # Compute baseline FIRST (no noise)
        print(f"\n  Baseline (No noise)")
        for noise_strength in noise_levels:
            ...
            for run_idx in range(NUM_RUNS):
                with torch.enable_grad():
                    heatmap = gradcam(test_image, target_class,
                                    channel_mask=None, noise_strength=0.0)
            ...
            baseline_road = np.mean(road_scores)
            all_images_results['Baseline'][0.0].append(baseline_road)
            print(f"ROAD={baseline_road:.4f}")

        # Fill other noise levels with same baseline
        for noise_strength in noise_levels:
            if noise_strength != 0.0:
                all_images_results['Baseline'][noise_strength].append(baseline_road)
        ...
        for interval_idx, interval_name in enumerate(interval_names):
            ...
            for noise_strength in noise_levels:
                if noise_strength == 0.0:
                    # σ=0 = baseline
                    all_images_results[interval_name][0.0].append(baseline_road)
                    continue
                ...
                road_scores = []
                for run_idx in range(NUM_RUNS):
                    # Add noise ONLY to this interval
                    with torch.enable_grad():
                        heatmap = gradcam(test_image, target_class,
                ...
                all_images_results[interval_name][noise_strength].append(avg_road)
    ...
    for interval_name in interval_names:
        interval_results = {}
        for noise_strength in noise_levels:
            image_scores = all_images_results[interval_name][noise_strength]
            mean_road = np.mean(image_scores)
            std_road = np.std(image_scores)

            interval_results[noise_strength] = {
                'road': mean_road,
                'std': std_road,
            }
        ...
        print(f"\n{interval_name}:")
        for noise_strength in noise_levels:
            road = interval_results[noise_strength]['road']
            std = interval_results[noise_strength]['std']
    ...
    save_path = "./results/channel_sensitivity_selective_noise.png"

    plot_selective_noise_sensitivity(results_dict, save_path)
    ...
    print("- Which channel interval is most critical for explanation quality?")
    print("- High-activation or low-activation channels more sensitive to noise?")
    print()
